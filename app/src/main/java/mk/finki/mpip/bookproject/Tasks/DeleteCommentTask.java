 ...
 */
public class DeleteCommentTask extends AsyncTask<String, Void, Integer> {

    Context context;
    private RestTemplate restTemplate;
    private DeleteCommentListener listener;

    public DeleteCommentTask(Context ctx,DeleteCommentListener listener){
        context = ctx;
    ...
    protected Integer doInBackground(String... params) {
        ...
        String url = context.getResources().getString(R.string.url_comment_delete);
        url += commentId;
        ...
        ResponseEntity<Integer> response =
                template.exchange(url, HttpMethod.GET, null, Integer.class);
    ...
    protected void onPostExecute(Integer result) {
        ...
        listener.DeleteCommentTaskFinished(result);
    }
        ...
        if (netInfo != null && netInfo.isConnected()) {
            try {
                ...
                if (urlConnection.getResponseCode() == 200) {        // 200 = "OK" code (http connection is fine).
                    Log.v("testTag","Delete Comment Avaible..");
...
//            restTemplate.getMessageConverters().add(converter);
    ...
    public interface DeleteCommentListener{
        void DeleteCommentTaskFinished(Integer result);
    }
}
